...
if (initParams.tgWebAppData?.length) {
...
if (initParams.tgWebAppThemeParams?.length) {
...
function byteLength(str) {
  ...
  for (var i = str.length - 1; i >= 0; i--) {
    ...
    else if (code > 0x7ff && code <= 0xff_ff) s += 2;
...
const BackButton = (() => {
  ...
  backButton.show = () => setParams({ is_visible: true });
  backButton.hide = () => setParams({ is_visible: false });
  return backButton;
})();
...
const MainButton = (() => {
  ...
  mainButton.setText = (text) => mainButton.setParams({ text });
  ...
  mainButton.show = () => mainButton.setParams({ is_visible: true });
  mainButton.hide = () => mainButton.setParams({ is_visible: false });
  mainButton.enable = () => mainButton.setParams({ is_active: true });
  mainButton.disable = () => mainButton.setParams({ is_active: false });
...
const SettingsButton = (() => {
  ...
  settingsButton.show = () => setParams({ is_visible: true });
  settingsButton.hide = () => setParams({ is_visible: false });
  return settingsButton;
})();
...
const HapticFeedback = (() => {
  ...
  hapticFeedback.impactOccurred = (style) => triggerFeedback({ type: "impact", impact_style: style });
  hapticFeedback.notificationOccurred = (type) => triggerFeedback({ type: "notification", notification_type: type });
  hapticFeedback.selectionChanged = () => triggerFeedback({ type: "selection_change" });
  return hapticFeedback;
})();
...
const CloudStorage = (() => {
  ...
  cloudStorage.setItem = (key, value, callback) => invokeStorageMethod(
    "saveStorageValue",
    { key, value },
    callback
  );
  cloudStorage.getItem = (key, callback) => cloudStorage.getItems(
    [key],
    callback
      ? (err, res) => {
          if (err) callback(err);
  ...
  cloudStorage.getItems = (keys, callback) => invokeStorageMethod("getStorageValues", { keys }, callback);
  cloudStorage.removeItem = (key, callback) => cloudStorage.removeItems([key], callback);
  cloudStorage.removeItems = (keys, callback) => invokeStorageMethod("deleteStorageValues", { keys }, callback);
  cloudStorage.getKeys = (callback) => invokeStorageMethod("getStorageKeys", {}, callback);
  return cloudStorage;
})();
...
const BiometricManager = (() => {
  ...
  function onBiometryInfoReceived(eventType, eventData) {
    ...
    if (initRequestState.callbacks.length > 0) {
      for (const callback of initRequestState.callbacks) {
        callback();
      }
    }
  ...
  biometricManager.init = (callback) => {
    if (!checkVersion()) {
...
function onPopupClosed(eventType, eventData) {
  if (webAppPopupOpened) {
    ...
    const button_id = typeof eventData.button_id === "undefined" ? null : eventData.button_id;
...
function onQrTextReceived(eventType, eventData) {
  if (webAppScanQrPopupOpened) {
    ...
    const data = typeof eventData.data === "undefined" ? null : eventData.data;
...
function getRequestedContact(callback, timeout) {
  ...
  let reqDelay = 0;
  var reqInvoke = () => {
    invokeCustomMethod("getRequestedContact", {}, (err, res) => {
      if (res?.length) {
...
function invokeCustomMethod(method, params, callback) {
  ...
  const req_params = { req_id, method, params: params || {} };
...
WebApp.setHeaderColor = (color_key) => {
...
WebApp.isVersionAtLeast = (ver) => versionAtLeast(ver);
WebApp.onEvent = (eventType, callback) => {
  onWebViewEvent(eventType, callback);
};
WebApp.offEvent = (eventType, callback) => {
...
WebApp.switchInlineQuery = (query = "", choose_chat_types) => {
  ...
  if (choose_chat_types) {
    ...
    for (const chat_type of choose_chat_types) {
  ...
  if (typeof params.buttons === "undefined") {
    ...
    for (const button of params.buttons) {
      ...
      if (
        ...
        var text = typeof button.text === "undefined" ? "" : strTrim(button.text);
        if (!text.length) {
          console.error(
...
WebApp.invokeCustomMethod = (method, params, callback) => {
  invokeCustomMethod(method, params, callback);
};
WebApp.ready = () => {
